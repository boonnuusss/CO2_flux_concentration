...
land_filename = [...
"netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_185201-185212.nc";
...
"netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r1i1p1f1_gr_185201-185212.nc";
...
"netAtmosLandCO2Flux_Emon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_185001-186912.nc";
...
for i = 1:length(land_filename)
    ...
    hold on
    title("land CO2 flux in 1852")
    xlabel('Latitude')
    ylabel('CO2 flux (PgC yr^-^1)')
    
end
legend
saveas(gcf, 'land_CO2_flux_line1852.png')
...
saveas(gcf, 'land_CO2_flux_Bar1852.png')

save output_land_plot1852.mat CO2_flux_land_year_areas CO2_flux_land_year_LON lat_land model
...
function [time_land,lat_land,lon_land,CO2_land,model] = select_parameter(filepath,filename)
    ...
    is_1852=time_land>365+365&time_land<=365+365+366;
    time_land = time_land(is_1852);
    lat_land = land.lat.data;
    lon_land = land.lon.data;
    CO2_land = -land.netAtmosLandCO2Flux.data(is_1852,:,:);
    is_error = abs(CO2_land)>9.9e19;
    CO2_land(is_error)=nan;
    model = string(land.source_id);

    if ~(length(time_land)==12||length(time_land)==13)
        error('数据时间范围选取错误')
    end
...
function [time_land,lat_land,lon_land,CO2_flux_land_year,model] = calculate_CO2_flux_land_year(filepath,land_filename, land_fraction_filename, land_areacella_filename)
    ...
    CO2_flux_land_year = squeeze(sum(CO2_flux_land,1,"omitnan"))*365*24*60*60/(time_size*1e12);
end
