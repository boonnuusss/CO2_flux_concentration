...
land_filename = [...
"netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_189901-189912.nc";
"netAtmosLandCO2Flux_Emon_CanESM5_historical_r5i1p1f1_gn_185001-201412.nc";
"netAtmosLandCO2Flux_Emon_CESM2_historical_r11i1p1f1_gn_185001-189912.nc";
...
"netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r1i1p1f1_gr_189901-189912.nc";
...
for i = 1:length(land_filename)
    ...
    hold on
    title("land CO2 flux in 1899")
    xlabel('Latitude')
    ylabel('CO2 flux (PgC yr^-^1)')
    
end
legend
saveas(gcf, 'land_CO2_flux_line1899.png')
...
saveas(gcf, 'land_CO2_flux_Bar1899.png')

save output_land_plot1899.mat CO2_flux_land_year_areas CO2_flux_land_year_LON lat_land model
...
function [time_land,lat_land,lon_land,CO2_land,model] = select_parameter(filepath,filename)
    ...
    if filename == "netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_189901-189912.nc"...
            || filename=="netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r11i1p1f1_gr_189901-189912.nc"...
            || filename=="netAtmosLandCO2Flux_Emon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_189001-190912.nc"...
            || filename=="netAtmosLandCO2Flux_Emon_CESM2_historical_r11i1p1f1_gn_185001-189912.nc"
    ...
    if filename== "netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_189901-189912.nc"...
            || filename== "netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r11i1p1f1_gr_189901-189912.nc"
        is_1899=(time_land<=365);
    elseif filename== "netAtmosLandCO2Flux_Emon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_189001-190912.nc"
        day_begin = datenum([1899 1 1 0 0 0])-datenum([1890 1 1 0 0 0]);
        day_end = datenum([1900 1 1 0 0 0])-datenum([1890 1 1 0 0 0]);
        is_1899=(day_begin<=time_land)&(time_land<=day_end);
    else
        day_begin = datenum([1899 1 1 0 0 0])-datenum([1850 1 1 0 0 0]);
        day_end = datenum([1900 1 1 0 0 0])-datenum([1850 1 1 0 0 0]);
        is_1899=(day_begin<=time_land)&(time_land<=day_end);
    end
    time_land = time_land(is_1899);
    lat_land = land.lat.data;
    lon_land = land.lon.data;
    CO2_land = -land.netAtmosLandCO2Flux.data(is_1899,:,:);
