...
land_filename = [...
"netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_190101-190112.nc";
"netAtmosLandCO2Flux_Emon_CanESM5_historical_r5i1p1f1_gn_185001-201412.nc";
"netAtmosLandCO2Flux_Emon_CESM2_historical_r11i1p1f1_gn_190001-194912.nc";
...
"netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r1i1p1f1_gr_190101-190112.nc";
"netAtmosLandCO2Flux_Emon_GFDL-ESM4_historical_r1i1p1f1_gr1_185001-194912.nc";
"netAtmosLandCO2Flux_Emon_MIROC-ES2H_historical_r1i1p4f2_gn_185001-201412.nc"; % grid_fixed
"netAtmosLandCO2Flux_Emon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_189001-190912.nc";
"netAtmosLandCO2Flux_Emon_UKESM1-0-LL_historical_r1i1p1f2_gn_185001-194912.nc"];
...
for i = 1:length(land_filename)
    ...
    hold on
    title("land CO2 flux in 1901")
    xlabel('Latitude')
    ylabel('CO2 flux (PgC yr^-^1)')
    
end
legend
saveas(gcf, 'land_CO2_flux_line1901.png')
...
saveas(gcf, 'land_CO2_flux_Bar1901.png')

save output_land_plot1901.mat CO2_flux_land_year_areas CO2_flux_land_year_LON lat_land model
...
function [time_land,lat_land,lon_land,CO2_land,model] = select_parameter(filepath,filename)
    ...
    if filename == "netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_190101-190112.nc"...
            || filename=="netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r11i1p1f1_gr_190101-190112.nc"...
            || filename=="netAtmosLandCO2Flux_Emon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_189001-190912.nc"...
            || filename=="netAtmosLandCO2Flux_Emon_CESM2_historical_r11i1p1f1_gn_190001-194912.nc"
    ...
    if filename== "netAtmosLandCO2Flux_Emon_AWI-ESM-1-1-LR_historical_r1i1p1f1_gn_190101-190112.nc"...
            || filename== "netAtmosLandCO2Flux_Emon_EC-Earth3-CC_historical_r11i1p1f1_gr_190101-190112.nc"
        startYear=1901;
        is_1901 = cal_target_year_index(startYear,targetYear,time_land);
    elseif filename== "netAtmosLandCO2Flux_Emon_CESM2_historical_r11i1p1f1_gn_190001-194912.nc"
        startYear=1900;
        is_1901 = cal_target_year_index(startYear,targetYear,time_land);
    elseif filename== "netAtmosLandCO2Flux_Emon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_189001-190912.nc"
        startYear=1890;
        is_1901 = cal_target_year_index(startYear,targetYear,time_land);
    else
        startYear=1850;
    ...
    time_land = time_land(is_1901);
    lat_land = land.lat.data;
    lon_land = land.lon.data;
    CO2_land = -land.netAtmosLandCO2Flux.data(is_1901,:,:);
...
function [time_land,lat_land,lon_land,CO2_flux_land_year,model] = calculate_CO2_flux_land_year(filepath, land_filename, land_fraction_filename, land_areacella_filename)
    ...
    sftlf_fix = repmat(sftlf,[1,1,size(CO2_land,1)]);
    sftlf_fix = permute(sftlf_fix,[3,1,2]);
    areacella_fix = repmat(areacella,[1,1,size(CO2_land,1)]);
    areacella_fix = permute(areacella_fix,[3,1,2]);
    % mydisp(size(sftlf_fix))
    % disp(size(CO2_land))

    CO2_flux_land = CO2_land.*sftlf_fix/100.*areacella_fix;
    CO2_flux_land_year = squeeze(sum(CO2_flux_land,1,"omitnan"))*365*24*60*60/(size(CO2_land,1)*1e12);
end
...
function is_year = cal_target_year_index(startYear,targetYear,time_land)
    time0 = datenum([startYear 1 1 0 0 0]);
    day_begin = datenum([targetYear 1 1 0 0 0])-time0;
    day_end = datenum([targetYear+1 1 1 0 0 0])-time0;
    is_year=(day_begin<=time_land)&(time_land<=day_end);
end