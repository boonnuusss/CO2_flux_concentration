...
"fgco2_Omon_EC-Earth3-CC_historical_r1i1p1f1_gn_200301-200312.nc";
"fgco2_Omon_GFDL-ESM4_historical_r1i1p1f1_gr_199001-200912.nc";
"fgco2_Omon_MIROC-ES2H_historical_r1i1p4f2_gn_185001-201412.nc";
"fgco2_Omon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_199001-200912.nc";
...
add_data = [-0.57;-1.01;0.62;-1.08;-0.73];
...
for i = 1:length(ocean_filename)
    ...
    hold on
    title("ocean CO2 flux in 2003")
    xlabel('Latitude')
    ylabel('CO2 flux (PgC yr^-^1)')
    
end
legend
saveas(gcf, 'ocean_CO2_flux_line2003.png')
% 对CO2_flux_ocean_year_area数据画柱状图，使用Bar函数
h=figure(1);
% 调整图窗大小
set(h,'position',[100,100,1200,600])
CO2_flux_ocean_year_areas = [CO2_flux_ocean_year_areas,add_data];
model = [model,"Keeling"];
...
saveas(gcf, 'ocean_CO2_flux_Bar2003.png')
...
function [time_ocean,lat_ocean,lon_ocean,CO2_ocean,model] = select_parameter(filepath,filename)
    ...
    elseif filename=="fgco2_Omon_GFDL-ESM4_historical_r1i1p1f1_gr_199001-200912.nc"
    ...
    if filename == "fgco2_Omon_CESM2_historical_r1i1p1f1_gn_185001-201412.nc"...
            || filename=="fgco2_Omon_EC-Earth3-CC_historical_r1i1p1f1_gn_200301-200312.nc"...
            || filename== "fgco2_Omon_GFDL-ESM4_historical_r1i1p1f1_gr_199001-200912.nc"...
            || filename== "fgco2_Omon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_199001-200912.nc"...
    ...
    if filename== "fgco2_Omon_EC-Earth3-CC_historical_r1i1p1f1_gn_200301-200312.nc"
        is_2003=(time_ocean<365);
   elseif filename== "fgco2_Omon_UKESM1-0-LL_historical_r1i1p1f2_gn_195001-201412.nc"
            is_2003=(19359<time_ocean)&(time_ocean<19723);
   elseif filename== "fgco2_Omon_GFDL-ESM4_historical_r1i1p1f1_gr_199001-200912.nc"...
            || filename== "fgco2_Omon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_199001-200912.nc"
       is_2003=(4745<time_ocean)&(time_ocean<5109);
    else
        is_2003=(55519<time_ocean)&(time_ocean<55883);
    end

    time_ocean = time_ocean(is_2003);
    CO2_ocean = -ocean.fgco2.data(is_2003,:,:);
...
function [lat_ocean,CO2_flux_ocean_year_LON,model,CO2_flux_ocean_year_areas]=plot_CO2_flux_ocean_year(filepath,ocean_filename, ocean_areacello_filename)
    ...
    if ocean_filename=="fgco2_Omon_MPI-ESM-1-2-HAM_historical_r1i1p1f1_gn_199001-200912.nc"
