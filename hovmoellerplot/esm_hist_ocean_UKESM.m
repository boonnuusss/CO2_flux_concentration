...
ocean_filename = ["fgco2_Omon_UKESM1-0-LL_esm-hist_r1i1p1f2_gn_185001-194912.nc";
                  "fgco2_Omon_UKESM1-0-LL_esm-hist_r1i1p1f2_gn_195001-201412.nc"];
ocean_areacello_filename = "areacello_Ofx_UKESM1-0-LL_piControl_r1i1p1f2_gn.nc";

%% load the data and plot the bar and line figure
time_ocean = [];
CO2_flux_ocean = [];
for i = 1:length(ocean_filename)
    [time_ocean_i,lat_ocean,lon_ocean,CO2_flux_ocean_i,model] = ...
        calculate_CO2_flux_ocean_year(filepath,ocean_filename(i),ocean_areacello_filename);
    time_ocean = [time_ocean, time_ocean_i];
    CO2_flux_ocean = cat(1,CO2_flux_ocean,CO2_flux_ocean_i);
end
...
function [time_ocean,lat_ocean,lon_ocean,CO2_ocean,model] = select_parameter(filepath,filename)
    ...
    lon_ocean = ocean.longitude.data(1,:);
